...
from config import FILE_DATA_PATH, client
...
def make_time_close_days(time_now_tmp) -> str: # new_time = start_time + timedelta(hours=25)
...
if not os.path.exists(FILE_DATA_PATH):
    with open(FILE_DATA_PATH, 'w') as csvfile:
        ...
        print(f'file {FILE_DATA_PATH} is created.\n')
            ...
            try:
                ...
                if data['RECOMMENDATION'] == 'STRONG_BUY' and symbol not in longs and int(data['NEUTRAL']) <= 8:
                    send_message(f'{symbol} + BUY\nCOUNT: {count_predicts}\nNEUTRAL: {data["NEUTRAL"]}\nBUY: '
                                 f'{data["BUY"]}\nSELL: {data["SELL"]}')
                    with open(FILE_DATA_PATH, 'a') as csvfile:
